...
const handlebook = async (e) => {
  ...
  try {
    ...
    // Step 3: Booking successful, proceed to payment
    const pnr = bookingResponse.data.data.Response.PNR;
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          // Step 4: Payment successful, update booking with payment details (if needed)
          const paymentUpdateResponse = await axios.post(`${apilink}/update-payment`, {
            pnr: pnr,
          ...
          if (paymentUpdateResponse.data?.status === "success") {
            Swal.fire({
              ...
              text: `Your flight has been booked! PNR: ${pnr}`,
          ...
          Swal.fire({
            icon: "error",
            title: "Payment Recorded, Booking Issue",
            text: "Payment was successful, but there was an issue updating the booking. Please contact support with PNR: " + pnr,
          });
        }
      },
      prefill: {
