...
const Page = ({ setActiveTab, fdatas, price }) => {
  ...
  useEffect(() => {
    const fetchUserData = async (userId) => {
      try {
        const token = JSON.parse(localStorage.getItem("NextGenUserToken"));
    ...
    if (!token) {
      ...
      fetchUserData(token); // Assuming token contains user ID
    }
  ...
  useEffect(() => {
    const initialPassengers = () => {
      if (!fdatas?.data?.FareBreakdown) {
        console.warn("No FareBreakdown data available");
        return;
      }
      ...
      setShowForms(passengers.map(() => true));
    };
  ...
  const addTraveler = () => {
    const maxPassengers = fdatas?.data?.FareBreakdown?.reduce(
      (sum, fare) => sum + fare.PassengerCount,
      0
    ) || 9; // Default to 9 if undefined
    if (passengers.length >= maxPassengers) {
      Swal.fire({
        icon: "warning",
        title: "Maximum Travelers Reached",
        text: `You cannot add more than ${maxPassengers} travelers.`,
        confirmButtonText: "OK",
      });
      return;
    }
  ...
  const handlePayment = async (bookingData) => {
    ...
    if (!amount || amount <= 0) {
      Swal.fire({
        ...
        text: "The payment amount is invalid. Please try again.",
        confirmButtonText: "OK",
      });
    ...
    try {
      ...
      const response = await axios.post(
        `${apilink}/create-razorpay-order`,
        {
          amount: amount * 100,
          currency: "INR",
          receipt: `receipt_${bookingData.PNR}`,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY || "rzp_test_Bi57EMsQ6K7ZZH",
        amount: amount * 100,
        currency: "INR",
        name: "Flight Booking",
        description: `Payment for PNR: ${bookingData.PNR}`,
        order_id: order_id,
        ...
        prefill: {
          name: userInfo?.name || `${passengers[0]?.FirstName} ${passengers[0]?.LastName}`,
          email: userInfo?.email || passengers[0]?.Email,
  ...
  const handlebook = async (e) => {
    ...
    if (!fdatas?.data?.FareBreakdown || !fdatas?.ResultIndex) {
      Swal.fire({
        icon: "error",
        title: "Invalid Flight Data",
        text: "Flight data is missing. Please search for flights again.",
        confirmButtonText: "OK",
      }).then(() => {
    ...
    try {
      ...
      if (response?.data?.status === "success") {
        const bookingData = response.data.data.Response;
        setBookingResponse(bookingData);
        Swal.fire({
          icon: "success",
          title: "Booking Successful",
          text: `Your flight has been booked successfully! PNR: ${bookingData.PNR}`,
        ...
        Swal.fire({
          ...
          text: response.data?.message || "An error occurred while processing your booking.",
          confirmButtonText: "OK",
        });
      }
    } catch (error) {
  ...
  const formatDateTime = (dateTimeStr) => {
    if (!dateTimeStr) return { formattedDate: "N/A", formattedTime: "N/A" };
          ...
          <table className="w-full mb-6">
            ...
            <tbody>
              <tr className="border-b">
                ...
                <td className="p-2 text-blue-600">{PNR || "N/A"}</td>
              </tr>
              <tr className="border-b">
                <td className="p-2 font-semibold">Booking ID</td>
                <td className="p-2 text-blue-600">{BookingId || "N/A"}</td>
              </tr>
              <tr className="border-b">
              ...
              <tr className="border-b">
                ...
                <td className="p-2 text-blue-600">{passenger?.ContactNo || "N/A"}</td>
              </tr>
              <tr className="border-b">
                <td className="p-2 font-semibold">Email</td>
                <td className="p-2 text-blue-600">{passenger?.Email || "N/A"}</td>
              </tr>
              <tr className="border-b">
                <td className="p-2 font-semibold">Trace ID</td>
                <td className="p-2 text-blue-600">{traceId || "N/A"}</td>
              </tr>
              <tr className="border-b">
                <td className="p-2 font-semibold">Token</td>
                <td className="p-2 text-blue-600">{token || "N/A"}</td>
              </tr>
              <tr className="border-b">
                <td className="p-2 font-semibold">User IP</td>
                <td className="p-2 text-blue-600">{userIp || "N/A"}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            {segments.length > 0 ? (
              segments.map((segment, index) => (
                <table key={index} className="w-full mb-4">
                  <thead>
                  ...
                  <tbody>
                    ...
                    <tr className="border-b">
                      ...
                      <td className="p-2">{segment.Airline?.FlightNumber || "N/A"}</td>
                    </tr>
                    <tr className="border-b">
                      <td className="p-2 font-semibold">Departure</td>
                      <td className="p-2">
                        {segment.Origin?.Airport?.CityName} ({segment.Origin?.Airport?.AirportCode || "N/A"}) at{" "}
                        {segment.Origin?.DepTime ? new Date(segment.Origin.DepTime).toLocaleString() : "N/A"}
                      </td>
                    </tr>
                    <tr className="border-b">
                      <td className="p-2 font-semibold">Arrival</td>
                      <td className="p-2">
                        {segment.Destination?.Airport?.CityName} ({segment.Destination?.Airport?.AirportCode || "N/A"}) at{" "}
                        {segment.Destination?.ArrTime ? new Date(segment.Destination.ArrTime).toLocaleString() : "N/A"}
                      </td>
                    </tr>
                  </tbody>
                </table>
  ...
  if (!fdatas?.data?.Segments?.[0]?.[0]) {
    return (
      <div className="text-center p-4">
        <p className="text-red-500">No flight data available. Please search again.</p>
        <button
          className="bg-[#DA5200] text-white px-4 py-2 rounded-md mt-4"
  ...
  return (
    <div className="">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex justify-between w-full">
                  ...
                  <span className="text-sm md:text-xl font-medium text-red-700">
                    {fdatas?.addat && new Date() - new Date(fdatas.addat) > 11 * 60 * 1000 && (
                      <span>Token is Expired, Search flight again</span>
                    )}
                  </span>
                </div>
              </div>
              ...
              <div className="rounded-sm border px-3 py-4 relative space-y-5">
                ...
                <div className="flex items-center gap-3 text-md md:text-xl">
                  ...
                  <div className="flex items-center gap-1">
                    <h4 className="">
                      {fdatas.data.Segments[0][0].Origin?.Airport?.CityName || "N/A"} -{" "}
                      {fdatas.data.Segments[0][0].Destination?.Airport?.CityName || "N/A"}
                    </h4>
                ...
                <div className="flex gap-5 flex-col md:flex-row items-start justify-between space-y-4 lg:space-y-0 lg:space-x-4">
                  <div className="flex items-center gap-4">
                    <img
                      src={`/images/${fdatas.data.Segments[0][0].Airline?.AirlineCode || "default"}.png`}
                      alt="Airline logo"
                      className="h-10 w-10 rounded-lg"
                      onError={(e) => (e.target.src = "/images/default.png")} // Fallback image
                    />
                    <div>
                      <p className="text-sm md:text-lg">{fdatas.data.Segments[0][0].Airline?.AirlineName || "N/A"}</p>
                      <p className="text-xs">
                        {fdatas.data.Segments[0][0].Airline?.AirlineCode || "N/A"}-
                        {fdatas.data.Segments[0][0].Airline?.FlightNumber || "N/A"}
                      </p>
                      <p className="text-xs">
                        {["All", "Economy", "PremiumEconomy", "Business", "PremiumBusiness", "First"][
                          fdatas.data.Segments[0][0].CabinClass - 1
                        ] || "Unknown"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex w-full gap-2 justify-between md:w-[70%] md:px-3">
                    <div className="flex flex-col gap-1 items-start">
                      ...
                      <div className="flex flex-col text-xs">
                        <span className="font-bold text-nowrap">
                          {fdatas.data.Segments[0][0].Origin?.Airport?.CityName || "N/A"} (
                          {fdatas.data.Segments[0][0].Origin?.Airport?.AirportCode || "N/A"})
                        </span>
                        <span>{formatDate(fdatas.data.Segments[0][0].Origin?.DepTime)}</span>
                        <span>
                          Terminal {fdatas.data.Segments[0][0].Origin?.Airport?.Terminal || "N/A"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 items-center">
                      <p className="text-xs">
                        {Math.floor(fdatas.data.Segments[0][0].Duration / 60)}h-
                        {fdatas.data.Segments[0][0].Duration % 60}m
                      </p>
                    ...
                    <div className="flex flex-col gap-1 items-start">
                      ...
                      <div className="flex flex-col text-xs">
                        <span className="text-nowrap font-bold">
                          {fdatas.data.Segments[0][0].Destination?.Airport?.CityName || "N/A"} (
                          {fdatas.data.Segments[0][0].Destination?.Airport?.AirportCode || "N/A"})
                        </span>
                        <span>{formatDate(fdatas.data.Segments[0][0].Destination?.ArrTime)}</span>
                        <span>
                          Terminal {fdatas.data.Segments[0][0].Destination?.Airport?.Terminal || "N/A"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="border-2">
                  ...
                  <div className="flex justify-between items-start p-2">
                    <div className="flex justify-start items-center w-[35%] gap-6">
                      ...
                      <div>
                        <h6 className="text-black text-sm font-semibold capitalize">
                          {fdatas.data.Segments[0][0].Airline?.AirlineName || "N/A"}
                        </h6>
                    ...
                    <div className="text-black text-sm font-semibold capitalize w-[28%]">
                      {fdatas.data.Segments[0][0].Baggage || "N/A"}
                    </div>
                    <div className="text-black text-sm font-semibold capitalize w-[20%]">
                      {fdatas.data.Segments[0][0].CabinBaggage || "N/A"}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="priceBoxAndDetails border rounded shadow-lg">
              ...
              <div className="flex justify-between px-3 py-3 text-sm border-b">
                ...
                <p className="flex items-center font-bold text-xs">
                  <FaRupeeSign />
                  {fdatas.data.Fare?.OfferedFare || "N/A"}
                </p>
              </div>
            </div>
            ...
            <div className="booking flex justify-center items-center mt-3">
              <button
                className={`bg-[#DA5200] text-sm lg:text-lg tracking-normal text-white rounded-full w-1/2 md:w-[80%] py-2 flex justify-center items-center ${
                  isLoading || paymentLoading ? "opacity-75 cursor-not-allowed" : ""
                }`}
                onClick={handlebook}
      ...
      {showModal && <BookingConfirmationModal bookingResponse={bookingResponse} onClose={closeModal} />}
    </div>
  );
};
